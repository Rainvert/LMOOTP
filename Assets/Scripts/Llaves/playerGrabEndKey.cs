...
public class playerGrabEndKey : MonoBehaviour
{
    ...
    void OnTriggerEnter2D(Collider2D col){
        if(col.tag == "Player" && !wasPicked){
            ...
            FindObjectOfType<GameplayManager>().setRondaFinal(true);
            FindObjectOfType<GameplayManager>().showFinalAlert();

            FindObjectOfType<BGMManager>().activateBGM(BGMType.Batalla);
        }
    }
